            ...
            $i = 1;
     		while ($reg = $query_Tipo->fetch_object()) {
     			...
     			$data[] = array("id"=>$i,
					...
					"3"=>$reg->nombre,
					"4"=>$reg->descripcion,
					"5"=>'<span id="'.$reg->idarticulo.'">'.$reg->objetivo.'</span>',
					'<button class="btn btn-primary" data-toggle="modal" data-target="#myModal" data-id="'.$reg->idarticulo.'" data-name="'.$reg->nombre.'">
						  Añadir Objetivo
						</button>');
				$i++;
			}
